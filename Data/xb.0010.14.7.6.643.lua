...
function Container:OpenChildren(...)
	...
	for i, id in ipairs({...}) do
		if (type(id) == 'table') then
			items[i] = {Item.GetItemIDFromDualInput(id[1]), id[2]}
		else
			items[i] = {Item.GetItemIDFromDualInput(id), false}
		end
	end
    ...
    for _, id in ipairs(items) do
        for i = 0, self:ItemCount()-1 do
            ...
            if (item.id == id[1]) then
                ...
                while (self:UseItem(i) ~= 1) do wait(300) end
...
Self.OpenLocker = function()
    ...
    wait(500, 1000)
    Container.GetLast():OpenChildren({3497, false, true}, {3498, false, true}, {3499, false, true}, {3500, false, true})
end
                    ...
                    for spot = 0, bp:ItemCount() - 1 do -- loop through all the items in loot backpack
                        ...
                        if (data) then -- the item is in the deposit list
                            ...
                            elseif(not Container.GetByName("Depot Chest"):isOpen())then -- this slot has not been recursed AND depot is closed :(
                                local reopen = Self.OpenDepot() -- try to reopen depot
